...
def render_html(config: dict[str, Any], cards: list[Card]) -> str:
    ...
    holo_opacity = min(0.5, max(0.0, float(appearance.get("holographic_opacity", 0.18))))
    data = json.dumps({
...
.pocket img{{display:block;width:100%;height:100%;object-fit:contain;border-radius:7px}}.holographic img{{filter:saturate(1.05) contrast(1.02)}}
.holographic::after{{content:"";position:absolute;inset:8px;border-radius:7px;pointer-events:none;background:linear-gradient(115deg,transparent 14%,rgba(103,232,249,.18) 31%,rgba(244,114,182,.15) 43%,rgba(253,224,71,.16) 55%,rgba(167,139,250,.14) 66%,transparent 82%);background-size:220% 220%;mix-blend-mode:screen;opacity:{holo_opacity};animation:holo-shift 9s ease-in-out infinite}}
@keyframes holo-shift{{0%,100%{{background-position:100% 20%}}50%{{background-position:0 80%}}}}@media(prefers-reduced-motion:reduce){{.holographic::after{{animation:none;background-position:50% 50%}}}}
