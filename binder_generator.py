...
def render_html(config: dict[str, Any], cards: list[Card]) -> str:
    ...
    holo_darkening = min(0.65, max(0.0, float(appearance.get("holographic_darkening", 0.35))))
    data = json.dumps({
...
.pocket img{{display:block;width:100%;height:100%;object-fit:contain;border-radius:7px}}.holographic{{--shine-x:25%;--shine-y:18%;border-color:rgba(165,243,252,.8);box-shadow:0 0 22px rgba(103,232,249,.24),0 0 32px rgba(244,114,182,.12);transition:border-color .2s,box-shadow .2s}}.holographic:hover{{border-color:rgba(255,255,255,.95);box-shadow:0 0 28px rgba(103,232,249,.4),0 0 42px rgba(244,114,182,.25)}}.holographic img{{filter:saturate(1.08) contrast(1.04)}}
.holographic::before,.holographic::after{{content:"";position:absolute;z-index:1;inset:8px;border-radius:7px;pointer-events:none}}
.holographic::before{{background:rgb(62,76,18);mix-blend-mode:multiply;opacity:{holo_darkening};-webkit-mask-image:linear-gradient(to bottom,#000 0 9.5%,transparent 9.5% 48%,#000 48% 100%);mask-image:linear-gradient(to bottom,#000 0 9.5%,transparent 9.5% 48%,#000 48% 100%)}}
.holographic::after{{background-image:radial-gradient(circle at var(--shine-x) var(--shine-y),rgba(255,255,255,.95) 0 4%,rgba(255,255,255,.55) 9%,rgba(255,255,255,.08) 24%,transparent 38%),linear-gradient(125deg,rgba(255,35,90,.62),rgba(255,205,40,.58) 20%,rgba(40,255,180,.55) 40%,rgba(40,175,255,.64) 60%,rgba(190,70,255,.6) 80%,rgba(255,35,90,.62)),repeating-linear-gradient(115deg,transparent 0 10px,rgba(255,255,255,.2) 11px,transparent 13px);background-size:100% 100%,260% 260%,100% 100%;mix-blend-mode:color-dodge;opacity:{holo_opacity};animation:holo-shift 4.5s ease-in-out infinite}}
@keyframes holo-shift{{0%,100%{{background-position:0 0,110% 5%,0 0}}50%{{background-position:0 0,-10% 95%,0 0}}}}@media(prefers-reduced-motion:reduce){{.holographic::after{{animation:none;background-position:0 0,50% 50%,0 0}}}}
...
function render(){{const pages=DATA[layout.value],data=pages[index];pageSelect.value=index;document.querySelector('#title').textContent=`Page ${{index+1}} — ${{data.section}}`;document.querySelector('#count').textContent=`${{index+1}} / ${{pages.length}}`;grid.innerHTML='';data.pockets.forEach(card=>{{const el=document.createElement('article');el.className='pocket'+(card?'':' empty');if(!card)el.textContent='EMPTY';else{{if(HOLO_VARIANTS.has(card.variant)){{el.classList.add('holographic');el.addEventListener('pointermove',event=>{{const box=el.getBoundingClientRect();el.style.setProperty('--shine-x',`${{((event.clientX-box.left)/box.width)*100}}%`);el.style.setProperty('--shine-y',`${{((event.clientY-box.top)/box.height)*100}}%`)}});el.addEventListener('pointerleave',()=>{{el.style.removeProperty('--shine-x');el.style.removeProperty('--shine-y')}})}}const img=document.createElement('img');img.src=card.url;img.alt=`#${{String(card.n).padStart(3,'0')}} ${{card.name}}`;img.onerror=()=>{{el.innerHTML=`<div class="error">Image failed to load<br>${{card.url}}</div>`}};const tag=document.createElement('div');tag.className='tag';tag.innerHTML=`<strong>#${{String(card.n).padStart(3,'0')}}</strong> ${{card.name}}<br>${{card.variant}}`;el.append(img,tag)}}grid.append(el)}});document.querySelector('#prev').disabled=index===0;document.querySelector('#next').disabled=index===pages.length-1}}
