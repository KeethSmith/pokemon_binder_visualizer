...
button,select{{color:var(--ink);background:#243449;border:1px solid #4a607a;border-radius:8px;padding:8px 11px;font-weight:650}}button:hover{{border-color:var(--accent);cursor:pointer}}
...
.pocket img{{display:block;width:100%;height:100%;object-fit:contain;border-radius:7px}}.tag{{position:absolute;left:8px;right:8px;bottom:8px;padding:6px 7px;border-radius:6px;color:white;background:rgba(5,8,12,.88);font-size:12px;text-align:center}}.tag strong{{color:var(--accent)}}
.empty{{border-style:dashed;color:#627187;font-weight:700;letter-spacing:.12em}}.error{{padding:14px;color:#ffb4b4;text-align:center;overflow-wrap:anywhere}}.nav{{margin-top:16px;display:flex;justify-content:space-between;gap:12px}}.note{{margin-top:18px;font-size:13px}}
@media(max-width:700px){{.page{{gap:6px;padding:8px}}.tag{{inset:auto 4px 4px;font-size:9px;padding:3px}}}}@media print{{header,.nav,.note{{display:none}}body{{background:white;color:black}}main{{margin:0}}.page{{box-shadow:none;break-after:page}}}}
</style></head><body>
<header><div class="bar"><h1>{title} · Binder</h1><label>Layout <select id="layout"><option value="paired">Paired</option><option value="split">Split</option></select></label><label>Page <select id="pageSelect"></select></label></div></header>
<main><p class="summary">{escape(summary)}</p><div class="page-head"><h2 id="title"></h2><span id="count"></span></div><section class="page" id="page"></section><div class="nav"><button id="prev">← Previous</button><button id="next">Next →</button></div><p class="note">Images load from the URL template configured for this set. Pocket labels identify card variants that share an image.</p></main>
<script>const DATA={data};const layout=document.querySelector('#layout'),pageSelect=document.querySelector('#pageSelect'),grid=document.querySelector('#page');let index=0;
...
function render(){{const pages=DATA[layout.value],data=pages[index];pageSelect.value=index;document.querySelector('#title').textContent=`Page ${{index+1}} — ${{data.section}}`;document.querySelector('#count').textContent=`${{index+1}} / ${{pages.length}}`;grid.innerHTML='';data.pockets.forEach(card=>{{const el=document.createElement('article');el.className='pocket'+(card?'':' empty');if(!card)el.textContent='EMPTY';else{{const img=document.createElement('img');img.src=card.url;img.alt=`#${{String(card.n).padStart(3,'0')}} ${{card.name}}`;img.onerror=()=>{{el.innerHTML=`<div class="error">Image failed to load<br>${{card.url}}</div>`}};const tag=document.createElement('div');tag.className='tag';tag.innerHTML=`<strong>#${{String(card.n).padStart(3,'0')}}</strong> ${{card.name}}<br>${{card.variant}}`;el.append(img,tag)}}grid.append(el)}});document.querySelector('#prev').disabled=index===0;document.querySelector('#next').disabled=index===pages.length-1}}
