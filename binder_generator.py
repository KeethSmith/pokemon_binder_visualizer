...
def render_html(config: dict[str, Any], cards: list[Card]) -> str:
    ...
    holo_opacity = min(0.65, max(0.0, float(appearance.get("holographic_opacity", 0.2))))
...
.pocket img{{display:block;width:100%;height:100%;object-fit:contain;border-radius:7px}}.holographic img{{filter:saturate(.96) contrast(1.02)}}
.holographic::before,.holographic::after{{content:"";position:absolute;z-index:1;inset:8px;border-radius:7px;pointer-events:none;-webkit-mask-image:linear-gradient(to bottom,#000 0 9.5%,transparent 9.5% 48%,#000 48% 100%);mask-image:linear-gradient(to bottom,#000 0 9.5%,transparent 9.5% 48%,#000 48% 100%)}}
.holographic::before{{background:rgb(62,76,18);mix-blend-mode:multiply;opacity:{holo_darkening}}}
.holographic::after{{background-image:linear-gradient(112deg,transparent 12%,rgba(255,255,255,.2) 32%,rgba(220,245,160,.3) 43%,rgba(255,255,255,.14) 53%,transparent 70%),repeating-linear-gradient(115deg,transparent 0 15px,rgba(255,255,255,.1) 16px,transparent 18px);background-size:250% 250%,100% 100%;mix-blend-mode:screen;opacity:{holo_opacity};animation:holo-shift 6.5s ease-in-out infinite}}
@keyframes holo-shift{{0%,100%{{background-position:110% 10%,0 0}}50%{{background-position:-10% 90%,0 0}}}}@media(prefers-reduced-motion:reduce){{.holographic::after{{animation:none;background-position:50% 50%,0 0}}}}
