...
.tag{{position:absolute;z-index:1;left:8px;right:8px;bottom:8px;padding:6px 7px;border-radius:6px;color:white;background:rgba(5,8,12,.9);font-size:12px;text-align:center}}.tag strong{{color:var(--accent)}}.price{{color:#8ff0b2;font-weight:750;white-space:nowrap}}
...
<main><p class="summary" id="summary"></p><div class="page-head"><h2 id="title"></h2><span id="count"></span></div><nav class="page-nav" aria-label="Binder pages"><button id="prev">← Previous</button><button id="next">Next →</button></nav><section class="page" id="page"></section><p class="note">Click any card to view it larger. Prices are TCGplayer market prices from TCGCSV's daily API export. Card images load from Pokémon's official image CDN.</p></main>
...
<script>
const DATASETS={datasets},PRICE_CATALOG={prices};let active=DATASETS[0],activeFormatId='3x3',index=0;
const setSelect=document.querySelector('#setSelect'),binderFormat=document.querySelector('#binderFormat'),layout=document.querySelector('#layout'),pageSelect=document.querySelector('#pageSelect'),grid=document.querySelector('#page'),cardDialog=document.querySelector('#cardDialog'),modalCard=document.querySelector('#modalCard'),modalImage=document.querySelector('#modalImage'),modalTitle=document.querySelector('#modalTitle'),modalPrice=document.querySelector('#modalPrice');
...
function cardPrice(card){{return PRICE_CATALOG.sets?.[active.id]?.cards?.[String(card.n)]?.[card.variant]||null}}
...
function openCard(card,isHolographic){{const price=cardPrice(card);modalImage.src=card.url;modalImage.alt=`#${{String(card.n).padStart(3,'0')}} ${{card.name}}`;modalTitle.textContent=`#${{String(card.n).padStart(3,'0')}} ${{card.name}} — ${{card.variant}}`;modalPrice.replaceChildren();if(price&&price.url){{const link=document.createElement('a');link.href=price.url;link.target='_blank';link.rel='noopener';link.textContent=`TCGplayer Market: ${{priceText(price)}}`;modalPrice.append(link)}}else modalPrice.textContent='TCGplayer market price unavailable';modalCard.className='modal-card'+(isHolographic?' holographic':'');cardDialog.showModal()}}
function render(pages=currentPages()){{const data=pages[index];if(!data){{grid.innerHTML='<div class="error">This set has no pages.</div>';return}}pageSelect.value=String(index);document.querySelector('#title').textContent=`${{active.name}} · Page ${{index+1}} — ${{data.section}}`;document.querySelector('#count').textContent=`${{index+1}} / ${{pages.length}}`;grid.innerHTML='';const holoVariants=new Set(active.holoVariants);data.pockets.forEach(card=>{{const el=document.createElement('article');el.className='pocket'+(card?' clickable':' empty');if(!card)el.textContent='EMPTY';else{{const isHolographic=holoVariants.has(card.variant),price=cardPrice(card);if(isHolographic)el.classList.add('holographic');el.tabIndex=0;el.setAttribute('role','button');el.setAttribute('aria-label',`Enlarge #${{String(card.n).padStart(3,'0')}} ${{card.name}} — ${{card.variant}} — TCGplayer ${{priceText(price)}}`);const show=()=>openCard(card,isHolographic);el.addEventListener('click',show);el.addEventListener('keydown',event=>{{if(event.key==='Enter'||event.key===' '){{event.preventDefault();show()}}}});const img=document.createElement('img');img.src=card.url;img.alt=`#${{String(card.n).padStart(3,'0')}} ${{card.name}}`;img.onerror=()=>{{el.innerHTML=`<div class="error">Image failed to load<br>${{card.url}}</div>`}};const tag=document.createElement('div');tag.className='tag';tag.innerHTML=`<strong>#${{String(card.n).padStart(3,'0')}}</strong> ${{card.name}}<br>${{card.variant}} · <span class="price">${{priceText(price)}}</span>`;el.append(img,tag)}}grid.append(el)}});document.querySelector('#prev').disabled=index===0;document.querySelector('#next').disabled=index===pages.length-1;syncUrl()}}
