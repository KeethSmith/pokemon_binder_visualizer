...
function currentPages(){{const pageSize=currentFormat().pageSize,pages=[];active.sections.forEach(section=>{{const pockets=orderedPockets(section);for(let offset=0;offset<pockets.length;offset+=pageSize){{const chunk=pockets.slice(offset,offset+pageSize);while(chunk.length<pageSize)chunk.push(null);pages.push({{section:section.name,pockets:chunk}})}}}});return pages}}
