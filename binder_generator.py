...
.card-surface{{position:relative;width:100%;aspect-ratio:660/921;flex:none;overflow:hidden;border-radius:7px}}
.card-surface img{{display:block;width:100%;height:100%;object-fit:contain;border-radius:inherit}}.holographic{{border-color:rgba(112,132,118,.72);box-shadow:0 0 18px rgba(0,0,0,.65),inset 0 0 12px rgba(120,140,125,.1)}}.holographic img{{filter:brightness(.928) saturate(.93) contrast(1.04)}}
.holographic::before,.holographic::after{{content:"";position:absolute;z-index:1;inset:0;border-radius:inherit;pointer-events:none}}
.holographic::before{{background:rgb(62,76,18);mix-blend-mode:multiply;opacity:var(--holo-darkening)}}
.holographic::after{{background-image:repeating-linear-gradient(115deg,transparent 0 10px,rgba(8,14,10,.2) 11px,rgba(225,238,230,.18) 12px,transparent 15px);background-size:100% 100%;mix-blend-mode:screen;opacity:var(--holo-opacity)}}
...
.finish-preview{{position:absolute;z-index:2;inset:0;border-radius:inherit;overflow:hidden;pointer-events:none;mix-blend-mode:multiply}}
...
<dialog id="cardDialog" aria-labelledby="modalTitle"><button class="modal-close" id="modalClose" aria-label="Close enlarged card">×</button><div class="modal-card card-surface" id="modalCard"><img id="modalImage" alt=""></div><p class="modal-title" id="modalTitle"></p><p class="modal-price" id="modalPrice"></p></dialog>
<script>
