...
@media(max-width:700px){{header{{padding:6px 9px}}h1{{flex-basis:100%;font-size:17px;text-align:center}}.bar{{justify-content:center;gap:5px 8px}}.bar label{{font-size:12px}}.bar select{{padding:4px 6px;font-size:12px}}.page{{gap:6px;padding:8px}}.tag{{inset:auto 4px 4px;font-size:9px;padding:3px}}}}@media print{{header,.page-nav,.note{{display:none}}body{{background:white;color:black}}main{{margin:0}}.page{{box-shadow:none;break-after:page}}}}
</style></head><body>
<header><div class="bar"><h1>Pokémon Binder Visualizer</h1><label>Set <select id="setSelect"></select></label><label>Binder <select id="binderFormat"></select></label><label>Spacing <select id="layout"><option value="paired">None</option><option value="split">Fresh page</option></select></label><label>Variants <select id="finishSelect"><option value="none">None</option></select></label><label>Page <select id="pageSelect"></select></label></div></header>
