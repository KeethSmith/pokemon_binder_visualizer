...
header{{position:sticky;top:0;z-index:2;padding:9px 14px;background:rgba(16,22,31,.94);backdrop-filter:blur(10px);border-bottom:1px solid #35465a}}
.bar{{max-width:1180px;margin:auto;display:flex;gap:12px;align-items:center;flex-wrap:wrap}}.header-row,.controls{{display:contents}}h1{{font-size:20px;margin:0 auto 0 0}}#controlsToggle{{display:none}}
button,select{{color:var(--ink);background:#243449;border:1px solid #4a607a;border-radius:8px;padding:6px 9px;font-weight:650;touch-action:manipulation}}button:hover{{border-color:var(--accent);cursor:pointer}}button:disabled{{opacity:.42;cursor:not-allowed}}.page-nav{{display:flex;justify-content:space-between;gap:12px;margin:0 0 12px}}
...
@media(max-width:700px){{header{{padding:6px 9px}}.bar{{display:block}}.header-row{{display:flex;align-items:center;justify-content:space-between;gap:8px}}h1{{margin:0;font-size:17px;white-space:nowrap}}#controlsToggle{{display:block;padding:4px 8px;font-size:12px}}.controls{{display:none;padding-top:6px;gap:5px 8px;align-items:center;justify-content:center;flex-wrap:wrap}}.controls.open{{display:flex}}.controls label{{font-size:12px}}.controls select{{padding:4px 6px;font-size:12px}}.page{{gap:2px;padding:4px}}.pocket{{padding:2px}}.tag{{inset:auto 4px 4px;font-size:9px;padding:3px}}.tag .card-name{{display:none}}}}@media print{{header,.page-nav,.note{{display:none}}body{{background:white;color:black}}main{{margin:0}}.page{{box-shadow:none;break-after:page}}}}
</style></head><body>
<header><div class="bar"><div class="header-row"><h1>Pokémon Binder Visualizer</h1><button type="button" id="controlsToggle" aria-controls="controls" aria-expanded="false">Options</button></div><div class="controls" id="controls"><label>Set <select id="setSelect"></select></label><label>Binder <select id="binderFormat"></select></label><label>Spacing <select id="layout"><option value="paired">None</option><option value="split">Fresh page</option></select></label><label>Variants <select id="finishSelect"><option value="none">None</option></select></label><label>Page <select id="pageSelect"></select></label></div></div></header>
...
const setSelect=document.querySelector('#setSelect'),binderFormat=document.querySelector('#binderFormat'),layout=document.querySelector('#layout'),finishSelect=document.querySelector('#finishSelect'),pageSelect=document.querySelector('#pageSelect'),grid=document.querySelector('#page'),controls=document.querySelector('#controls'),controlsToggle=document.querySelector('#controlsToggle'),cardDialog=document.querySelector('#cardDialog'),modalCard=document.querySelector('#modalCard'),modalImage=document.querySelector('#modalImage'),modalTitle=document.querySelector('#modalTitle'),modalPrice=document.querySelector('#modalPrice');
const SHOWCASE_LABELS={{pokeball:'Poké Ball',masterball:'Master Ball',loveball:'Love Ball',friendball:'Friend Ball',quickball:'Quick Ball',duskball:'Dusk Ball',teamrocket:'Team Rocket',energy:'Energy Symbol'}};
