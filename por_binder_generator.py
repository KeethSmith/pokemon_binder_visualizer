...
if __name__ == "__main__":
    ...
    configs = [read_config(root / "sets" / "perfect_order.json")]
    configs.extend(read_config(config_dir / f"{name}.json") for name in BUILTIN_ORDER)
    ...
    (root / "por_binder_layout.txt").write_text(
        render_text(primary_config, primary_cards),
        encoding="utf-8",
    )
